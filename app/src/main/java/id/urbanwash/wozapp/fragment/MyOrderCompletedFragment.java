...
public class MyOrderCompletedFragment extends Fragment implements MyOrderListener {
    ...
    public void onStart() {
        ...
        mOrders = Session.getCompletedOrders();

        if (mOrders == null) {
            mOrders = new ArrayList<OrderBean>();
        }

        mMyOrderAdapter = new OrderAdapter(mAppCompatActivity, Constant.ORDER_STATUS_INT_COMPLETED, this, mOrders);
    ...
    public void refreshCompletedOrders() {
        ...
        mOrders.clear();
        mOrders.addAll(Session.getCompletedOrders());
