...
import id.urbanwash.wozapp.adapter.ReportHourlyDeliveryAdapter;
...
public class ReportHourlyDeliveryActivity extends BaseActivity implements ReportDailyListener {
    ...
    private ReportHourlyDeliveryAdapter mReportHourlyDeliveryAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mDeliveryDate = (Date) getIntent().getSerializableExtra(Constant.DATA_DATE);

        if (mDeliveryDate == null) {
            mDeliveryDate = new Date();
        }

        mTitleLabel = (AppCompatTextView) getSupportActionBar().getCustomView().findViewById(R.id.label_title);
        mTitleLabel.setText(CommonUtil.formatDayShortDate(mDeliveryDate));
        ...
        mReportHourlyDeliveryAdapter = new ReportHourlyDeliveryAdapter(this, mReports);
        mReportView.setAdapter(mReportHourlyDeliveryAdapter);
    }
    ...
    public void onDateSelected(Date date) {
        ...
        Intent intent = new Intent(mAppCompatActivity, ReportHourlyDeliveryPlaceActivity.class);
    ...
    public void onAsyncGetReports(List<ReportBean> reports) {
        ...
        for (int i = Constant.EARLIEST_HOUR_FOR_ORDER; i <= Constant.LATEST_HOUR_FOR_ORDER; ) {
            ...
            Long time = mDeliveryDate.getTime() + i * Constant.TIME_HOUR;
        ...
        mReportHourlyDeliveryAdapter.notifyDataSetChanged();
    ...
    public void getRecords() {
        ...
        if (Session.isTransporterUser()) {
            httpAsyncManager.getHourlyDeliveryReport(Session.getEmployee(), mDeliveryDate);
        } else {
            httpAsyncManager.getHourlyDeliveryReport(mDeliveryDate);
        }
    }
}
